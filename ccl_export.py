...
Flow:
  ...
  6. Download CSV
  7. Upload to S3  s3://payout-recon/ccl/txn/raw_daily/
...
S3_PREFIX  = os.environ.get("S3_CCL_PREFIX", "ccl/txn/raw_daily/")
...
def notify_slack(message: str, color: str = "good"):
    """Send a formatted Slack message (export summary / error alerts)."""
    ...
    try:
        r = requests.post(
            ...
            json={"channel": channel, "text": f"{icon} {message}",
                  "attachments": [{"color": color, "text": message, "footer": "CCL Exporter"}]},
            headers=headers, timeout=10,
        )
...
async def login(page) -> bool:
    ...
    # Step 3: Authy 2FA — send "Please Approve" every 1 min, check every 30s, 5 loops max
...
async def export_transactions(page) -> Path:
    ...
    if await tx_link.count() > 0:
        ...
        raise RuntimeError("Transactions Report link not found")
    ...
    # 4. Pick dates via VueDatePicker calendar
    ...
    if await download_btn.count() > 0:
        ...
        raise RuntimeError("Download CSV button not found")
...
async def main():
    ...
    print(f"[*] S3      : {S3_ENABLED}  ({S3_BUCKET}/{S3_PREFIX})")
    print("=" * 60)
